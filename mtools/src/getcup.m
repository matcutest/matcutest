...
% feasibility_list.m is a function that returns a cell array containing the names of all feasibility problems
mfiledir = fileparts(mfilename('fullpath'));  % The directory where this .m file resides.
feasibility_list = fullfile(mfiledir, 'feasibility_list.m');
% Open the files for writing.
txtid = fopen(probinfotxt, 'wt');
texid = fopen(probinfotex, 'wt');
listid = fopen(problist, 'wt');
flistid = fopen(feasibility_list, 'wt');
if txtid == -1 || texid == -1 || listid == -1 || flistid == -1
    error('MatCUTEst:FailToOpenFile', 'Failed to open probinfo.txt, probinfo.tex, problist, or feasibility_list.m.');
end
% Write the header lines of probinfo.txt, probinfo.tex, problist, and feasibility_list.m
...
try
    ...
    for iprob = 1 : nprob
        ...
        % Record the names of feasibility problems in feasibility_list.m
        if probinfo{iprob}.is_feasibility
            fprintf(flistid, '    ''%s'', ...\n', probinfo{iprob}.name);
        end
    end
catch getcu_error
